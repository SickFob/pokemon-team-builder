...
import { deleteTeamById } from '../actions/team';
...
const DeleteForm = ({ id }: { id: number }) => {
  const [state, formAction] = useFormState(deleteTeamById, initialState);
  ...
  return (
    <>
      {
        showToastState === true && (
          <CustomToast type={state?.success ? 'success' : 'danger'} message={state?.message} />
        )
      }
